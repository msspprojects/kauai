...
```
```{r}
model2 <- lm(acres~air_temp_set_1+wind_speed_set_1+wind_direction_set_1+wind_gust_set_1+dew_point_temperature_set_1d, data = df_selected)
summary(model2)
```
```{r}
model3 <- lm(acres~air_temp_set_1+wind_direction_set_1+dew_point_temperature_set_1d, data = df_selected)
summary(model3)
```
```{r}
...
print(mse)
```
model2 gives us the best results, which means that the important predictors are air temperature, wind direction and dew point temperature.

Use random forest to verify the results:
```{r}
